...
def load_fashion_mnist(batch_size=64): # load_fashion_mnist라는 이름의 함수를 정의하고, 이 함수는 기본적으로 batch_size 매개변수를 64로 설정합니다. 이 매개변수는 데이터를 얼마나 많은 단위로 나눌지 결정
    builtins.data_full = torchvision.datasets.FashionMNIST('./data',download=True, train=True, transform=ToTensor()) # torchvision 라이브러리의 datasets 모듈을 사용하여 FasionMNIST 데이터셋을 불러옵니다. './data'는 데이터셋이 저장될 경로를 지정하며, download=True는 해당 경로에 데이터가 없을 경우 인터넷에서 자동으로 다운로드하도록 설정합니다. train=True는 학습용 데이터셋을 불러오는 것을 의미하고, transform=ToTensor()는 데이터셋의 이미지들을 파이토치 텐서로 변환하는 함수를 적용
    # 전체 데이터셋을 학습용, 검증용, 테스트용으로 분할합니다. (6:2:2)비율 사용
    train_size = int(0.6 * len(data_full)) # train data의 비율을 0.6을준다
    val_size = int(0.2 * len(data_full)) # val data의 비율을 0.2를 준다
    test_size = len(data_full) - train_size - val_size # test data의 비율을 남은 비율을 준다. 
...
def plot_results(hist): # plot_results라는 함수를 정의하는데 hist라는 이름의 딕셔너리를 매개변수로 받는데 학습과 검증 과정의 정확도와 손실이 배열 형태로 저장되어 있음
    ...
    plt.plot(hist['test_acc'], label='Test acc') # 그래프에 test데이터의 정확도도 표시되게끔 추가하였습니다. 
    ...
    plt.plot(hist['test_loss'], label='Test loss') # 그래프에 test데이터의 손실도 표시되게끔 추가하였습니다. 
